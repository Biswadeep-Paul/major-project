...
const DoctorsList = () => {
  const { doctors, toggleDoctorAccess, aToken, getAllDoctors, changeAvailability,getRatingsData } = useContext(AdminContext)
    ...
    const fetchAllRatings = async () => {
      ...
      for (const doctor of doctors) {
        try {
          ...
          console.error(`Error fetching ratings for doctor ${doctor._id}:`, error)
        }
      }
  ...
  const generateStars = (rating) => {
    const stars = []
    const starCount = Math.min(Math.round(rating || 0), 5)
      ...
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
          {doctors.map((item, index) => {
            const averageRating = doctorRatingsMap[item._id] || 0
